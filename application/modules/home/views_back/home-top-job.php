...
<?php if(!empty($corporate_job)){ ?>
    <?php foreach ($corporate_job as $key => $value) {
        ...
        <div class="cj-item">
            ...
            <?php */} ?>
            <?php if(isset($eid) && $eid>0){?>
            <div class="item-info" style="min-height: 75px;">
                <strong><?php echo $orgname; if($orgname=="Care Nepal") echo " - ".$eid; ?></strong>
                <ul>
                    <?php
             $related_job = $this->home_model->get_related_top_job_by_id($value->eid);
    ...
    ?>
    <div class="item-info" style="float: right; padding-right: 10px;">
        <a class="apply" href="<?php echo base_url(). 'corporate_jobs';?>">View All</a>
    </div>
    <?php
}
?>