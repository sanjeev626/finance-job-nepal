...
class Employer_model extends CI_Model {
    ...
    public function updatePostJob($logo,$eid,$jid){
        ...
        //echo $this->db->last_query();
        //echo $jobtitle.' --> '.$jobcategory.' --> '.$salaryrange.' --> '.$job_url.'<br>';
        $this->sendEmailToJobseeker($jobcategory,$salaryrange,$jobtitle,$job_url);
    }

    function sendEmailToJobseeker($jobcategory,$salaryrange,$jobtitle,$job_url){
        $limit = 10;
        ...
        if ($query->num_rows() == 0) {
            ...
            foreach($jobseekers as $key => $jobseeker)
            {
                ...
                <body>
                  Dear '.$toName.',<br>
                    You might be interested to apply for '.$jobtitle.'<br>
                    Please <a href="'.$job_url.'">click here</a> to know more about the job.<br><br>
                    Thank you<br>
                    '.$fromName.'<br>
                </body>
                </html>
