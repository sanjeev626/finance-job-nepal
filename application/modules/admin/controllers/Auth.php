...
//echo "here we are";exit;		
	...
	// log the user in
	function jobseeker_console_login()
	{ 
		//print_r($_POST);
		...
		if ($this->form_validation->run() == true)
		{
			...
			//echo $this->input->post('password');exit;
			$remember=1;
			if ($this->ion_auth->jobseeker_consol_login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
			...
			else
			{
				...
				// redirect them back to the login page
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect('admin/seeker/check_login?'.$page, 'refresh'); // use redirects instead of loading views for compatibility with MY_Controller libraries
		...
		else
		{
			...
			$this->_render_page('seeker/check_login?'.$page, $this->data);
			//$this->session->set_flashdata('message', $this->ion_auth->errors());
			//redirect('admin/seeker/check_login', 'refresh');
		}
	}
